...
-- Helper: deterministic supplier code (uppercase alnum, first 5 chars)
CREATE OR REPLACE FUNCTION public._supplier_code(_dostawca_id text)
RETURNS text LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path=public AS $$
DECLARE r record; s text;
BEGIN
  SELECT kod_zrodlowy, alias_dostawcy, nazwa_dostawcy_original INTO r
  FROM public.dostawcy WHERE dostawca_id = _dostawca_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'Dostawca nie istnieje: %', _dostawca_id; END IF;
  s := COALESCE(NULLIF(btrim(r.kod_zrodlowy),''), NULLIF(btrim(r.alias_dostawcy),''), NULLIF(btrim(r.nazwa_dostawcy_original),''));
  IF s IS NULL THEN RAISE EXCEPTION 'Dostawca % nie ma kodu/alias/nazwy', _dostawca_id; END IF;
  s := upper(regexp_replace(s, '[^A-Za-z0-9]', '', 'g'));
  IF length(s) = 0 THEN RAISE EXCEPTION 'Nie można wyprowadzić kodu dostawcy z danych dostawcy %', _dostawca_id; END IF;
  RETURN substring(s, 1, 5);
END $$;

-- Helper: ISO3 from loading country
CREATE OR REPLACE FUNCTION public._kraj_iso3(_kraj_id text)
RETURNS text LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path=public AS $$
DECLARE v text;
BEGIN
  IF _kraj_id IS NULL OR btrim(_kraj_id) = '' THEN
    RAISE EXCEPTION 'Kraj załadunku wymagany dla numeru dostawy';
  END IF;
  SELECT NULLIF(btrim(iso3),'') INTO v FROM public.kraje WHERE kraj_id = _kraj_id;
  IF v IS NULL OR length(v) <> 3 THEN
    RAISE EXCEPTION 'Brak prawidłowego ISO3 w public.kraje dla kraju załadunku %', _kraj_id;
  END IF;
  RETURN upper(v);
END $$;

-- Helper: generate business number with advisory locks (no D/YYYY fallback)
CREATE OR REPLACE FUNCTION public._gen_business_numer(_dostawca_id text, _kraj_id text)
RETURNS text LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE
  v_code text := public._supplier_code(_dostawca_id);
  v_iso3 text := public._kraj_iso3(_kraj_id);
  v_sup_seq int;
  v_iso_seq int;
  v_num text;
BEGIN
  PERFORM pg_advisory_xact_lock(hashtextextended('dostawy_sup_'||_dostawca_id, 0));
  PERFORM pg_advisory_xact_lock(hashtextextended('dostawy_iso_'||v_iso3, 0));

  SELECT COUNT(*)+1 INTO v_sup_seq
    FROM public.dostawy
   WHERE dostawca_id = _dostawca_id
     AND numer_dostawy ~ ('^'||v_code||'/[0-9]{3}/[A-Z]{3}/[0-9]{3}$');

  SELECT COUNT(*)+1 INTO v_iso_seq
    FROM public.dostawy d
   WHERE d.numer_dostawy ~ ('^[A-Z0-9]+/[0-9]{3}/'||v_iso3||'/[0-9]{3}$');

  v_num := format('%s/%s/%s/%s', v_code, lpad(v_sup_seq::text,3,'0'), v_iso3, lpad(v_iso_seq::text,3,'0'));
  RETURN v_num;
END $$;

-- ============ utworz_dostawe_z_pozycjami ============
CREATE OR REPLACE FUNCTION public.utworz_dostawe_z_pozycjami(
  p_data_dostawy date, p_dostawca_id text, p_kraj_id text, p_import_manager_id text,
  p_status text, p_notes text, p_pozycje jsonb, p_data_zaladunku date DEFAULT NULL::date
...
BEGIN
  ...
  END IF;
  IF NOT ('super_admin' = ANY(v_role_keys)) AND p_import_manager_id IS DISTINCT FROM v_current_user THEN
  ...
  -- Validate ISO3 exists (raises with clear message if not)
  PERFORM public._kraj_iso3(p_kraj_id);

  -- Generate new business numer_dostawy
...
CREATE OR REPLACE FUNCTION public.aktualizuj_dostawe_z_pozycjami(
  p_dostawa_id uuid, p_data_dostawy date, p_data_zaladunku date, p_dostawca_id text,
  p_kraj_id text, p_import_manager_id text, p_status text, p_notes text, p_pozycje jsonb
) RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE
  v_existing record;
  v_role_keys text[];
...
BEGIN
  ...
  END IF;
  IF p_status NOT IN ('draft','planned') THEN RAISE EXCEPTION 'Nieprawidłowy status: %', p_status; END IF;
  IF NOT ('super_admin' = ANY(v_role_keys)) AND p_import_manager_id IS DISTINCT FROM v_existing.import_manager_id THEN
  ...
  -- For new-format deliveries, block changing supplier/loading country (would invalidate numer_dostawy)
  ...
  FOR v_pozycja IN SELECT * FROM jsonb_array_elements(p_pozycje) LOOP
    ...
    IF v_pos_id IS NOT NULL THEN
      ...
      SET produkt_id = v_pozycja->>'produkt_id',
          ...
          notes = NULLIF(v_pozycja->>'notes',''), updated_at = now()
